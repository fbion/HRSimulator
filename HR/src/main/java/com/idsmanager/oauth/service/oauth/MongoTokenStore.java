 ...
 */
package com.idsmanager.oauth.service.oauth;
 ...
 *
 * @author Shengzhao Li
 * @see org.springframework.security.oauth2.provider.token.JdbcTokenStore
 */
public class MongoTokenStore implements TokenStore, InitializingBean {
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        AccessToken accessToken = new AccessToken()
                ...
                .clientId(authentication.getAuthorizationRequest().getClientId())
    ...
    public Collection<OAuth2AccessToken> findTokensByUserName(String userName) {
        ...
        List<AccessToken> tokenList = tokenRepository.findAccessTokensByUsername(userName);
        for (AccessToken token : tokenList) {
            final OAuth2AccessToken accessToken = token.token();
            if (accessToken != null) {
                accessTokens.add(accessToken);
